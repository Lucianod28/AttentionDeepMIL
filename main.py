...
parser.add_argument("--reg", type=float, default=1e-3, metavar="R", help="weight decay")
parser.add_argument(
    "--target_number",
    type=int,
    default=9,
...
parser.add_argument(
    "--var_bag_length", type=int, default=2, metavar="VL", help="variance of bag length"
)
parser.add_argument(
    "--num_bags_train",
    type=int,
...
parser.add_argument(
    "--cancer", action="store_true", default=False, help="Use the breast cancer dataset"
)
parser.add_argument(
    "--model",
    type=str,
...
# hyperparameters
percentage_labeled = 0.8
# max_unsupervised_weight = (30 * (500* percentage_labeled)) / (500 - 50)
max_unsupervised_weight = 40
epoch_with_max_rampup = 60  # for rampup function
alpha = 0.6  # for weighting function
# disinformation_sampler = torch.distributions.bernoulli.Bernoulli(0.5)
disinformation_sampler = None
...
def train(train_loader, epoch, Z, z_tilde):
    model.train()
    supervised_loss = 0.0
    running_temporal_ensembling_loss = 0.0
    train_error = 0.0
    Y_probs = torch.zeros(len(train_loader))
    # reset gradients
    optimizer.zero_grad()
    for batch_idx, (data, label) in enumerate(train_loader):
        if disinformation_sampler is None:
            labeled = batch_idx % 100 < 100 * percentage_labeled
        else:
            labeled = True
            if batch_idx % 100 >= 100 * percentage_labeled:
                # Get random label
                label = disinformation_sampler.sample((1,))
        # labeled = True
        ...
        unsupervised_weight = (
            ramp_up_function(epoch, epoch_with_max_rampup) * max_unsupervised_weight
        )
        ...
        (
            ...
            Y_prob,
            neg_log_likelihood,
            temporal_ensembling_loss,
        ) = model.calculate_objective(
            data, bag_label, z_tilde[batch_idx], unsupervised_weight, labeled
        )
...
if __name__ == "__main__":
    ...
    print(
        "%d epochs, %d train bags, %d test bags, %d mean bag length, %d variance bag length"
        % (
            args.epochs,
            args.num_bags_train,
            args.num_bags_test,
    ...
    for epoch in range(1, args.epochs + 1):
        supervised_loss, temporal_ensembling_loss, train_error, Z, z_tilde = train(
            train_loader, epoch, Z, z_tilde
        )
        ...
        writer.add_scalar(
            "logs/temporal_ensembling_loss", temporal_ensembling_loss, epoch
        )
